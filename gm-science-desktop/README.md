...
`gm-science-desktop` 是 `gm-science` 的桌面客户端第一版，当前以本地模式为主，同时复用 `openppx` 的本地 client-api 和远程 gateway 接入准备。
...
`gm-science-desktop` 最理想的工作方式，是本地 `openppx client-api` 已经在跑：
...
如果 `gm-science-runtime` 不在 `gm-science-desktop` 的旁边，先设置：

```bash
export OPENPPX_ROOT=/path/to/gm-science-runtime
```
...
- 当前仓库的目标不是替代 `openppx` runtime，而是把它包装成本地科研工作台。
